/**
 * ============================================================
 * AVECO Robot Financiero — Charly (Bot IA Discord + Claude)
 * ============================================================
 ...
 *
 * Descripción:
 *   Agente financiero "Charly" corriendo 100% en Apps Script. Llama a
 *   la API de Anthropic (Claude) con tool_use y ejecuta las funciones
 *   del DataLake (04_DataLake.gs) por llamada directa, SIN HTTP, SIN
 *   Python y SIN n8n. Opera como bot de Discord vía polling REST.
 *
 * Flujo:
 *   [Trigger 1 min] charlyDiscordPoll() → lee canal → runCharly_()
 *      → Claude (tool_use) → executeTool_() → getBancarios()/etc.
 *      → responde en el canal
 *   [Trigger 7am]   charlyBriefingDiario() → briefing → embed a Discord
 *
 ...
 *   DISCORD_BOT_TOKEN   → Token del bot (NO el webhook)
 *   DISCORD_CHANNEL_ID  → Canal donde opera Charly
 *   SPREADSHEET_ID      → DataLake (usado por las funciones de negocio)
 *
 * Setup del bot Discord (una sola vez):
 *   1. discord.com/developers/applications → New Application "Charly AVECO"
 *   2. Bot → Add Bot → Reset Token → copiar → Script Property DISCORD_BOT_TOKEN
 *   3. OAuth2 → URL Generator → Scopes: bot → Permisos: Send Messages,
 *      Read Messages/View Channels, Read Message History → invitar al server
 *   4. Discord: Developer Mode ON → clic derecho al canal → Copy Channel ID
 *      → Script Property DISCORD_CHANNEL_ID
 *   5. Ejecutar setupCharly() una sola vez desde el editor
 *
 * NOTA DE DISEÑO (lineamientos AVECO):
 *   El estándar es notificar por Webhook. Charly es la EXCEPCIÓN
 *   justificada: necesita LEER y RESPONDER en el canal, lo que un
 *   webhook no permite. Por eso usa Bot Token. Las notificaciones de
 *   procesos (Importador/Renombrador/DataLake) siguen por webhook.
 *
 * DEPENDENCIAS INTERNAS:
 *   getConfig() / requireConfig_()  → 00_Config.gs
 *   sendDiscordNotification_        → 01_Notificaciones.gs (fallback)
 *   getStatus/getBancarios/etc.     → 04_DataLake.gs
 * ============================================================
 */
...
const CHARLY_TOOLS = [
  { name: 'get_status', description: 'Estado del DataLake: filas por hoja y movimientos pendientes de revisión.', input_schema: { type: 'object', properties: {}, required: [] } },
  { name: 'get_resumen_diario', description: 'Resumen diario: conciliación 30 días, bancarios 7 días, pendientes urgentes.', input_schema: { type: 'object', properties: {}, required: [] } },
  { name: 'get_sat_cfdis', description: 'CFDIs del SAT filtrados. Retorna uuid, nombre_emisor, total, fecha.', input_schema: { type: 'object', properties: { desde: { type: 'string', description: 'YYYY-MM-DD' }, hasta: { type: 'string', description: 'YYYY-MM-DD' }, limite: { type: 'integer' } }, required: [] } },
  { name: 'get_bancarios', description: 'Movimientos bancarios. Bancos válidos: BBVA, FONDEADORA, TC CLARA, TC KONFIO, DOLAR APP USD, SANTANDER.', input_schema: { type: 'object', properties: { desde: { type: 'string' }, hasta: { type: 'string' }, banco: { type: 'string' }, tipo: { type: 'string', description: 'EGRESO o INGRESO' }, limite: { type: 'integer' } }, required: [] } },
  { name: 'get_conciliacion', description: 'Reporte de conciliación básica (match exacto) SAT vs Board para un período.', input_schema: { type: 'object', properties: { desde: { type: 'string' }, hasta: { type: 'string' } }, required: [] } },
  { name: 'get_pending_review', description: 'Movimientos pendientes de revisión humana con tipo de problema detectado.', input_schema: { type: 'object', properties: { limite: { type: 'integer' } }, required: [] } },
  { name: 'save_decision', description: 'Guarda la decisión de Antonio sobre un movimiento: categoría, obra y estado.', input_schema: { type: 'object', properties: { id_interno: { type: 'string' }, categoria: { type: 'string' }, obra: { type: 'string', description: 'AM07, AM08, AM09, AM10, TULUM' }, estado: { type: 'string', description: 'revisado | aprobado | rechazado' } }, required: ['id_interno', 'estado'] } },
  { name: 'fuzzy_conciliar', description: 'Conciliación INTELIGENTE con fuzzy matching. Normaliza nombres de empresas (quita SA de CV, acentos), tolera ±5% en montos y ±7 días en fechas. Mejor que el match exacto para proveedores como Chakjuha, Construrent, Nohoch, etc.', input_schema: { type: 'object', properties: { desde: { type: 'string', description: 'YYYY-MM-DD (default: últimos 90 días)' }, hasta: { type: 'string', description: 'YYYY-MM-DD (default: hoy)' }, tolerancia_monto: { type: 'number', description: '0.0–1.0, default 0.05' }, similitud_nombre: { type: 'number', description: '0.0–1.0, default 0.70' } }, required: [] } },
];


// ============================================================
// SECCIÓN 2 — AGENTE PRINCIPAL (loop tool_use)
// ============================================================

/**
 * Ejecuta el agente Charly con tool_use de Claude.
 * Las tools llaman directamente a las funciones de 04_DataLake.gs (sin HTTP).
  ...
  for (let iter = 0; iter < CHARLY_MAX_ITER; iter++) {
    ...
    if (response.stop_reason === 'tool_use') {
      ...
      const toolResults = (response.content || [])
        ...
        .map(b => ({ type: 'tool_result', tool_use_id: b.id, content: executeTool_(b.name, b.input) }));
...
/**
 * Despacha un tool llamando directamente a la función del DataLake.
 */
...
// ============================================================
// SECCIÓN 3 — FUZZY MATCHING (nativo Apps Script)
// ============================================================
...
function ejecutarFuzzyConciliacion_(params) {
  ...
  cfdis.forEach(cfdi => {
    ...
    const entry = {
      ...
      match: { descripcion: m.b.descripcion || '', monto: Math.abs(parseFloat(m.b.monto || 0)), fecha: m.b.fecha || '', banco: m.b.banco || '', id_interno: m.b.id_interno || '' },
  ...
  return {
    ...
    resumen: {
      ...
      cobertura_pct: cfdis.length > 0 ? Math.round(confirmados.length / cfdis.length * 1000) / 10 : 0,
    },
  };
}
...
// ============================================================
// SECCIÓN 4 — INTEGRACIÓN ANTHROPIC
// ============================================================
...
function callClaudeAPI_(messages) {
  const cfg = requireConfig_(['ANTHROPIC_API_KEY']);
...
// ============================================================
// SECCIÓN 5 — INTEGRACIÓN DISCORD (Bot Token)
// ============================================================

/**
 * Lee los mensajes nuevos del canal (después de lastMsgId) usando el Bot Token.
 */
function fetchDiscordMessages_(lastMsgId) {
  ...
  return JSON.parse(resp.getContentText()).sort((a, b) => a.id.localeCompare(b.id));
}

/**
 * Envía un mensaje de texto al canal. Divide en chunks de 1900 caracteres.
 */
function sendDiscordMessageToChannel_(content, replyToMsgId) {
  ...
  const chunks = [];
  for (let i = 0; i < content.length; i += 1900) chunks.push(content.substring(i, i + 1900));

  chunks.forEach((chunk, idx) => {
    const payload = { content: chunk };
    if (idx === 0 && replyToMsgId) payload.message_reference = { message_id: replyToMsgId };
    UrlFetchApp.fetch(DISCORD_API_BASE + '/channels/' + cfg.DISCORD_CHANNEL_ID + '/messages', {
      method: 'POST',
      headers: { 'Authorization': 'Bot ' + cfg.DISCORD_BOT_TOKEN, 'Content-Type': 'application/json' },
...
/**
 * Envía un embed al canal usando el Bot Token. Si no hay token, cae al webhook.
 */
function sendDiscordEmbedCharly_(title, description, color, fields) {
  ...
  const embed = {
    ...
    footer:      { text: 'Charly · ' + cfg.PROJECT_LABEL + ' v' + cfg.VERSION },
  };
  ...
  // Fallback al webhook compartido (01_Notificaciones.gs).
  sendDiscordNotification_(title, description, color);
}


// ============================================================
// SECCIÓN 6 — HISTORIAL DE CONVERSACIÓN
// ============================================================
...
function saveHistory_(messages) {
  ...
  PropertiesService.getScriptProperties().setProperty(CHARLY_HISTORY_KEY, JSON.stringify(simplified));
}
 ...
 * Trigger time-based cada 1 minuto.
 */
  ...
  try {
    ...
    messages.forEach(msg => {
      ...
      try {
        ...
        sendDiscordMessageToChannel_('Lo siento, tuve un problema al procesar tu mensaje. Intenta de nuevo.', msg.id);
      }
 ...
 * Trigger time-based a las 7:00 AM (America/Mexico_City).
 * Se ejecuta 1h DESPUÉS de runDailySync (06:00) para usar datos frescos.
 */
function charlyBriefingDiario() {
  ...
  try {
    ...
    sendDiscordEmbedCharly_('☀️ Briefing Matutino AVECO — ' + fecha, text, 0x2ECC71, [
      { name: 'Modelo IA', value: getConfig().ANTHROPIC_MODEL, inline: true },
...
/**
 * Configura los triggers de tiempo de Charly. Ejecutar UNA SOLA VEZ.
 *   - charlyDiscordPoll → cada 1 minuto
 *   - charlyBriefingDiario → 07:00 (1h después de runDailySync)
 */
function setupCharly() {
  ...
  ScriptApp.newTrigger('charlyDiscordPoll').timeBased().everyMinutes(1).create();
  ...
  Logger.log('Triggers creados: charlyDiscordPoll (1 min) + charlyBriefingDiario (07:00)');

  sendDiscordEmbedCharly_(
    '🤖 Charly Online — AVECO Robot Financiero',
    'El agente financiero está activo y escuchando en este canal.\n\n' +
    'Escríbeme cualquier pregunta sobre las finanzas de AVECO:\n' +
...
// ============================================================
// SECCIÓN 9 — FUNCIONES DE PRUEBA MANUAL
// ============================================================
...
function testClaudeAPI() {
  try {
    ...
    Logger.log('Tokens: ' + JSON.stringify(resp.usage));
  } catch (e) {
...
function testDiscordRead() {
  ...
  msgs.forEach(m => Logger.log('  [' + m.id + '] ' + (m.author && m.author.username) + ': ' + (m.content || '').substring(0, 80)));
}

/** Envía un mensaje de prueba al canal usando el Bot Token. */
function testDiscordSend() {
  sendDiscordEmbedCharly_('🧪 Test Charly — Apps Script', 'Claude API + Discord Bot funcionando desde Google Apps Script.', 0x5865F2, [{ name: 'Estado', value: '✅ OK', inline: true }]);
