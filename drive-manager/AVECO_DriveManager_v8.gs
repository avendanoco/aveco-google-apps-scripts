...
function getRootFolderId_() {
  ...
  if (!id) throw new Error('No hay carpeta raíz configurada. Usa ⚙️ Configuración → Definir carpeta raíz (ID) en el menú.');
  return id;
}
...
function setRootFolderIdFromMenu() {
  ...
  if (!id) { ui.alert('⚠️ ID vacío. No se guardó nada.'); return; }
...
function contarContenidoCarpeta_(folder) {
  let total = 0;
  const f = folder.getFiles();   while (f.hasNext()) { f.next(); total++; }
  const s = folder.getFolders(); while (s.hasNext()) { s.next(); total++; }
  return { total };
}
...
/**
 * Índice Drive v8 — Columnas:
 * A  Est.          Auto      Estructura numérica de ruta (1.2.3)
 * B  Mov-Est.      Editable  Estructura destino para mover
 * C  Len           Fórmula   Longitud de Est.
 * D  Extensión     Dato      PDF, DOCX, CARPETA…
 * E  Nom Actual    Dato      Nombre en Drive
 * F  Nuevo Nom     Editable  Nuevo nombre para renombrar
 * G  Tags          Dato      Tags actuales
 * H  Tags Nuevo    Editable  Nuevas tags a aplicar
 * I  📝 Desc       Dato      Descripción actual
 * J  📝 Desc Nuevo Editable  Nueva descripción
 * K  Peso (KB)     Dato      Tamaño
 * L  Fecha Mod.    Dato      Última modificación
 * M  URL           Fórmula   Enlace directo
 * N  ID            Oculto    ID Drive
 * O  ID Padre      Oculto    ID carpeta padre
 * P  Lvl           Oculto    Nivel de profundidad
 * Q  Pfx           Oculto    Prefijo extraído
 * R  Estado        Resultado Resultado de operaciones
 */
function prepararIndiceSheet_() {
  ...
  ['B','F','H','J'].forEach(col => sheet.getRange(col + '2:' + col).setBackground('#fff9c4'));

  sheet.hideColumns(14); // N — ID
  sheet.hideColumns(15); // O — ID Padre
  sheet.hideColumns(16); // P — Lvl
  sheet.hideColumns(17); // Q — Pfx

  return sheet;
}

/**
 * Árbol de Carpetas v8 — Columnas:
 * A  Est.           Auto      Estructura numérica (1, 1.2, 1.2.3)
 * B  Mov-Est.       Editable  Estructura destino para mover carpeta
 * C  Len            Fórmula   Longitud de Est.
 * D  Nombre Carpeta Dato      Nombre actual
 * E  Nuevo Nom      Editable  Nuevo nombre de la carpeta
 * F  Items          Auto      Total archivos + subcarpetas directas
 * G  Tags           Dato      Tags actuales
 * H  Tags Nuevo     Editable  Nuevas tags
 * I  Desc           Dato      Descripción actual
 * J  Desc Nuevo     Editable  Nueva descripción
 * K  URL            Fórmula   Enlace directo
 * L  Lvl            Oculto    Nivel de profundidad
 * M  ID             Oculto    ID carpeta
 * N  Estado         Resultado Resultado de aplicar cambios
 */
function prepararArbolSheet_() {
  ...
  ['B','E','H','J'].forEach(col => sheet.getRange(col + '2:' + col).setBackground('#fff9c4'));

  sheet.hideColumns(12); // L — Lvl
  sheet.hideColumns(13); // M — ID

  return sheet;
}

// ============================================================
// INDEXAR DRIVE COMPLETO v8
// ============================================================
...
function scanFolderV8_(folder, lvl, estBase, indexRows, treeRows) {
  ...
  treeRows.push([
    estBase, '', '', nombreCarpeta, '', conteo.total,
    tags, '', descClean, '', folder.getUrl(), lvl, id, ''
  ]);
...
function scanFolderTreeOnlyV8_(folder, lvl, estBase, treeRows) {
  ...
  treeRows.push([estBase, '', '', folder.getName(), '', conteo.total,
    tags, '', descClean, '', folder.getUrl(), lvl, folder.getId(), '']);
...
// ============================================================
// ÁRBOL — RENOMBRAR CARPETAS (col E: Nuevo Nom)
// ============================================================
function renombrarCarpetasDesdeArbol() {
  const ss    = SpreadsheetApp.getActiveSpreadsheet();
  const sheet = ss.getSheetByName(TREE_SHEET);
  if (!sheet) { SpreadsheetApp.getUi().alert('No se encontró la hoja Árbol.'); return; }
...
// ============================================================
// ÁRBOL — MOVER CARPETAS (col B: Mov-Est.)
// ============================================================
function moverCarpetasDesdeArbolPorEstructura() {
  ...
  for (let i = 1; i < data.length; i++) {
    ...
    if (!destInfo) {
      sheet.getRange(i+1, 14).setValue('⚠️ Est. destino no encontrada: ' + movEst).setBackground('#fff3e0');
      continue;
    }
    if (destInfo.id === id) {
      sheet.getRange(i+1, 14).setValue('⚠️ Origen = Destino').setBackground('#fff3e0');
      continue;
    }
  ...
  escribirLog_(logSheet, logRows);
  SpreadsheetApp.getUi().alert('📦 Carpetas movidas: ' + movidas);
}

// ============================================================
// ÁRBOL — APLICAR TAGS / DESC (cols H y J)
// ============================================================
...
function previewOrganizacion() {
  ...
  if (!sheet) { SpreadsheetApp.getUi().alert('No se encontró el Índice.'); return; }
  const data = sheet.getDataRange().getValues();

  const mapaCarpetas = {};
  for (let i = 1; i < data.length; i++) {
    const ext = data[i][3]; const nombre = data[i][4]; const id = data[i][13];
  ...
  for (let i = 1; i < data.length; i++) {
    const ext = data[i][3]; const nombre = data[i][4];
...
// ============================================================
// ÍNDICE — ORGANIZAR POR EST. (col B: Mov-Est.)
// ============================================================
function organizarDesdeIndicePorEstructura() {
  const ui = SpreadsheetApp.getUi();
  if (ui.alert('🚀 Organizar por Est.',
...
// ============================================================
// ÍNDICE — RENOMBRAR (col F: Nuevo Nom)
// ============================================================
...
// ============================================================
// ÍNDICE — APLICAR TAGS / DESC (cols H y J)
// ============================================================
...
function eliminarItem() {
  const ss    = SpreadsheetApp.getActiveSpreadsheet();
  const sheet = ss.getSheetByName(INDEX_SHEET);
  if (!sheet) { SpreadsheetApp.getUi().alert('Abre primero el Índice Drive.'); return; }
  ...
  const ui = SpreadsheetApp.getUi();
  if (ui.alert('🗑️ Enviar a papelera',
      `¿Eliminar "${nombre}" (${ext})?\nEst.: ${est}`,
      ui.ButtonSet.YES_NO) !== ui.Button.YES) return;
...
function eliminarCarpetasVacias() {
  const ss    = SpreadsheetApp.getActiveSpreadsheet();
  const sheet = ss.getSheetByName(TREE_SHEET);
  if (!sheet) { SpreadsheetApp.getUi().alert('Primero sincroniza el Árbol.'); return; }
  ...
  if (!vacias.length) { SpreadsheetApp.getUi().alert('✅ No hay carpetas vacías.'); return; }

  const ui    = SpreadsheetApp.getUi();
  const lista = vacias.slice(0, 20).map((v, idx) => `${idx+1}. ${v.nombre}`).join('\n');
  if (ui.alert('🧹 ' + vacias.length + ' carpetas vacías encontradas',
  ...
  escribirLog_(logSheet, logRows);
  SpreadsheetApp.getUi().alert('🗑️ Eliminadas: ' + eliminadas + ' de ' + vacias.length);
}
